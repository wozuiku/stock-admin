...
<script>
	...
	export default {
		...
		methods:{
			...
			click(executeBatch, executeTime){
				console.log('drop.vue click executeBatch:', executeBatch, 'executeTime:', executeTime);
			...
			async strategyDrop(executeBatch, executeTime) {
				...
				console.log('strategy:', strategy, 'dropDay:', dropDay, 'dropDateFrom:', dropDateFrom)
				...
				//for (let i = 0; i < 1; i++) {
					...
					//获取符合策略股票列表
					historyDataList = await this.getHistoryData(dataList, dropDateFrom)
					console.log('historyDataList:', historyDataList);
			
					strategyItemList = this.getStrategyItems(historyDataList, dropDay)
					...
					this.$emit('getProcess', this.executeProcess)
				}
			},
			...
			//获取符合策略股票列表
			async getHistoryData(dataList, dropDateFrom) {
			...
			getStrategyItems(historyDataList, dropDay) {
