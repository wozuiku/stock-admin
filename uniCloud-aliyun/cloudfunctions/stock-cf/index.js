...
exports.main = async (event, context) => {
	...
	async function getPageNum(sector) {
		let url = 'https://q.10jqka.com.cn/index/index/board/' + sector + '/field/zdf/order/desc/page/1/ajax/1/'
	...
	async function getOnePage(sector, index) {
		let url = 'https://q.10jqka.com.cn/index/index/board/' + sector + '/field/zdf/order/desc/page/' + index + '/ajax/1/'
	...
	async function updateStock(stockCode, stockName) {
		const db = uniCloud.database();
		db.collection("stock").where({code: stockCode})
		  .update({
		    name: stockName,
			status: "N"
		  });
